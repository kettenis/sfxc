...
!
! 1.    PEPA
! 1.1   PEPA PROGRAM SPECIFICATION
! 1.1.1 PEPA adds entries to the table of contents for the PEP utility.
! 1.2   PEPA PROGRAM INTERFACE
! 1.2.1 CALLING SEQUENCE - CALL PEPA
!
! 1.2.2 COMMON BLOCKS USED
      INCLUDE 'ccon.i'
!            VARIABLES 'FROM':
!              1. KPEPC - The PEP module flow control flag.
!                          = 0 ==> Solar system geometry from JPL DE/LE405
!                                  ephemeris file (default). (Sun, Moon,
!                                  Earth, and all other planets except Pluto)
!
! 1.2.4 DATA BASE ACCESS -
!            ACCESS CODES ADDED:
!              1. 'PEP MESS'  -  THE DATA BASE ACCESS CODE FOR THE
...
!
! 1.2.9 PROGRAMMER - KATHY WATTS    04/25/77
...
!                    David Gordon 94.06.08 Corrected format statements, single
!                              and double quotes reversed.
!                    David Gordon 98.07.13 Deleted 'PEP 2000' Lcode.
!                    David Gordon 99.01.14 Added access codes 'PEP TAPE',
!                              'EARTH CE', 'SUN DATA', and 'MOONDATA' to
!                    Jim Ryan 2002 Sept. Interger*4 updates.
!
!     PEPA Program Structure
!
...
!
      DATA C_LPEPU /
     . 'PEP Routine - 2004.04.21 D. Gordon/GSFC,',
     . ' J2000 Ephemeris from JPL DE405.        '/
!
      DATA C_LPEPT /
...
!                    'ssd.jpl.nasa.gov/iau-comm4/access2ephs.html'
!                    and 'ftp://ssd.jpl.nasa.gov/pub/eph/export/'.
!
...
!                         day (days).
!             3.  TSKIP - Flag telling whether to compute new values or use
...
!     Real*8  EARTHD(3,3),SUND(3,2),XMOOND(3,2)
!     Real*8  DEARTH(3,3),DSUN(3,3),DXMOON(3,3)
...
!   Do the PUT's for the Earth, Sun, and Moon coordinates
      CALL PUT4 ( 'EARTH CE      ', EARTH, int2(3), int2(3), int2(1))
      CALL PUT4 ( 'SUN DATA      ', SUN, int2(3), int2(2), int2(1))
      CALL PUT4 ( 'MOONDATA      ', XMOON, int2(3), int2(2), int2(1))
!

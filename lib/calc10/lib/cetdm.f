      ...
      IMPLICIT None
!
!     ETDA adds entries to the table of contents for the Earth tide
!     module text message, the partial derivatives, and the contributions.
!
!     ETDA Program Interface:
!
!     Common blocks used:
      INCLUDE 'ccon.i'
!
...
!                              Module Text Message.
!              2. 'ETD CFLG' - The database access code for the Earth Tide
!                              Module flow control message.
!              3. 'ETD CONT' - The database access code for the Earth Tide
!                              Module contributions array.
!              4. 'ETJMGPAR' - The database access code for the Earth Tide
!                              Module partial derivatives array (1996 IERS
!                              model) as defined in John Gipson's 10 Aug 1998
!                              memo.
!              5. 'C82ETCON' - The database access code for the delay and
!                              rate contribution to get the previous Calc
!                              (versions 8.x) Earth tide model with third
!                              order term included.
!              6. 'ELASTCON' - The data base access code for the correction
!                              to change from the anelastic to the elastic
!                              case.
!              7. 'PERMDEF ' - The database access code for the delay and
!                              rate contribution to restore the effects of
...
!                                 implimented.
!                    David Gordon 94.01.21 Access code 'ETD3 CONT' added,
!                                 correction to remove third order tide.
!                    David Gordon 94.04.06 Changed to 'Implicit None'
!                    David Gordon 94.04.07 'ETD3CONT' changed to 'ETD IERS'.
!                                 Correction to use in SOLVE to get strict
!                                 equivalent of IERS Standards (1992) Solid
!                                 Earth Tides.
!                    David Gordon 94.04.27 'ETD3CONT', 'ETDKCONT, and 'ETD2CONT'
!                                 Lcodes added.
!                    David Gordon 94.07.12 Changed 'ETHLCONT' to 'ETD2CONT'.
!                    David Gordon 94.08.23 Removed 'ETD IERS' Lcode. Changed
!                                 default back to the IERS Standards. Updated
!                                 documentation.
!                    David Gordon 98.06.24 Update Lcodes for Calc 9.0.
!                                 'ETD PART' changed to 'ET96PART'. Deletes
!                                 added for old calc 8.x contributions.
!                    David Gordon 98.08.27 'ET96PART' changed to 'ETJMGPAR',
!                                 in case we need to blame John Gipson.
!                    David Gordon 99.01.06 Added Lcodes 'C82ETCON' and
!                                 'ELASTCON'. Mods for correlator use.
!                    David Gordon 99.10.15 Added Lcode 'PERMDEF ' to hold
!                                 contribution to restore the permanent
!                                 deformation effect.
!                    Jim Ryan     2002.09 Integer*2/4 conversion
!                    David Gordon 2003/2004 Delete C82ETCON, PERMDEF,
!                                 ELASTCON, and ETJMGPAR L-codes.
!
! 1.3   ETDA PROGRAM STRUCTURE
!
...
!            12. STAR(3)      - The J2000.0 source unit vector.
!            13. FA2K(14)     - The fundamental arguments (arcsec)
!            14. FAD2K(14)    - The CT time derivatives of the fundamental
!                               arguments. (arcsec/century)
!            15. CENT         - The number of Julian centuries elapsed since the
!                               epoch January 1.5, 2000. (centuries)
!            16. GEOLAT(2)    - The geocentric latitude at each site. (rad)
!
!           OUTPUT VARIABLES:
...
!
      INCLUDE 'cuser.i'
!           1. Calc_user - Analysis center (A) or Correlator (C) user
!           2. Use_tide  - For correlators only:
!                          = 'C8' => use IERS 1992 tide model + third order
!                                    term (tide free crust, pemanent tide
!                                    not removed).
!                          = 'C9' => use IERS 1996 tide model without
!                                    removing the permanent tide (as in
!                                    Calc 9.0)
!                          = 'RC' => use IERS 1996 tide model with
!                            permanent tide removed. (real crust frame)
!
!   Program specifications:
!
     ...
     .       XMOON(3,2), SITLON(2), STAR(3), EARTH(3,3), CENT,
...
!?????????????/
!      18. delta_r(3,2) - Sum of
!             First index runs over X,Y,Z; second runs over sites. (meters)
!      19. ddelta_r(3,2) - Time derivative of above, i.e., IERS tidal
!             velocities.
!             First index runs over X,Y,Z; second runs over sites. (m/sec)
!      20. delta_r3(3,2) - The third order tidal displacement at each site,
!             from the IERS 1996 model. First index runs over X,Y,Z;
!             second runs over sites. (meters)
!      21. ddelta_r3(3,2) - Time derivative of above, third order tidal
!             velocity.
!             First index runs over X,Y,Z; second runs over sites. (m/sec)
!
! 4.2.9 PROGRAMMER -
...
!
!  There are no partial! Keep subroutine for future use.
!
!     Normal conclusion.
      RETURN
      END
...
!           Access codes:
!             1. 'ETD CONT' - Database access code for the total Earth tide
!                             contributions.
!
!     External input/output - Possible debug output.
!

...
Correlator_node::Correlator_node(int rank) : Node(rank) {
  ...
  MPI_Send(&type, 1, MPI_INT, 0, MPI_TAG_COMMUNICATION, MPI_COMM_WORLD);
}
...
//         break;
//       }
    case MPI_TAG_ASK_COMMUNICATION_MEDIUM:
      { // An input reader is asking how to send the data to the node
        write_debug("An input reader is asking how to send the data to the node");
        int command;
        MPI_Recv(&command, 1, MPI_INT, status.MPI_SOURCE, 
             MPI_TAG_ASK_COMMUNICATION_MEDIUM, MPI_COMM_WORLD, &status2);
        assert(status.MPI_TAG == status2.MPI_TAG);
...
Correlator_node::open_input_channel_tcp(int rank_reader) {
  write_debug("TODO: open_input_channel_tcp");

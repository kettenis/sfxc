...
TCP_Connection::open_port(unsigned short int port, int connections) {
  ...
  if (interface == NULL) {
    ...
    DEBUG_MSG("Using the myri0 network");
    serverAddress.sin_family = AF_INET;
    serverAddress.sin_addr.s_addr = inet_addr(interface->ip().c_str());
    serverAddress.sin_port = htons(port);
  }
  ...
  if (verbose)
    std::cout << "  connected to " << inet_ntoa(serverAddress.sin_addr);
...
TCP_Connection::do_connect(uint64_t ip, unsigned short int port) {
  ...
  if (hostInfo == NULL) {
    //std::cout << __PRETTY_FUNCTION__ << "problem interpreting host: " << inet_ntoa(addr) << "\n";
    //return -1;
  }
